...
const Layout: React.FC<LayoutProps> = ({ activeTab, setActiveTab, children }) => {
  const tabs = [
    { id: AppTab.AI_ASSISTANT, label: 'Assistant', icon: (
    ...
    { id: AppTab.COMPONENT_SEARCH, label: 'Search', icon: (
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-slate-50">
      <header className="flex-none px-4 py-3 bg-indigo-600 text-white flex justify-between items-center shadow-lg">
        <h1 className="text-xl font-bold tracking-tight">EELIB</h1>
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex flex-col items-center p-2 transition-all duration-200 ${
                activeTab === tab.id ? 'text-indigo-600 transform scale-105' : 'text-slate-400'
              }`}
            >
              {tab.icon}
