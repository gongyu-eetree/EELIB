...
const AIAssistant: React.FC<AIAssistantProps> = ({ credits, onConsume, history, onSaveHistory }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <nav className="flex bg-white rounded-2xl p-1.5 shadow-sm border border-slate-200" aria-label="Feature navigation">
        <button onClick={() => setMode('IMAGE')} className={`flex-1 py-3 text-xs font-black rounded-xl transition-all ${mode === 'IMAGE' ? 'text-indigo-600 bg-indigo-50 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Image Analysis</button>
        <button onClick={() => setMode('CHAT')} className={`flex-1 py-3 text-xs font-black rounded-xl transition-all ${mode === 'CHAT' ? 'text-indigo-600 bg-indigo-50 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Tech Consult</button>
        <button onClick={() => setMode('HISTORY')} className={`flex-none px-6 py-3 text-xs font-black rounded-xl transition-all ${mode === 'HISTORY' ? 'text-indigo-600 bg-indigo-50 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>History</button>
      </nav>

      {error && (
        <div role="alert" className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center space-x-3">
      ...
      {mode === 'IMAGE' && (
        <>
          <section className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100">
            <h2 className="sr-only">Image Upload Section</h2>
            <div 
              onClick={() => fileInputRef.current?.click()} 
              className={`border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center cursor-pointer hover:bg-indigo-50/30 transition-all focus-within:ring-4 focus-within:ring-indigo-500/10 ${image ? 'py-4' : 'py-16'}`}
            >
              {image ? (
                <div className="relative">
                   <img src={image} alt="Hardware to analyze" className="max-h-64 rounded-3xl shadow-2xl border-4 border-white" />
                   <button 
                ...
                <div className="text-center space-y-4">
                  ...
                  <div className="flex flex-col space-y-1">
                    <p className="text-slate-500 font-black text-sm uppercase tracking-tight">Upload or Take a Picture</p>
                    <p className="text-slate-300 font-bold text-[10px] uppercase tracking-widest">Supports Schematic / PCB / Component</p>
                  </div>
                </div>
              )}
            </div>
            <input type="file" ref={fileInputRef} className="hidden" aria-label="Image file upload" accept="image/*" onChange={handleFileChange} />
          </section>

          <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
            <h2 className="text-[10px] font-black text-slate-400 mb-6 uppercase tracking-[0.2em] text-center">Select AI Workflow</h2>
            <div className="flex flex-wrap gap-2 mb-8 justify-center" role="radiogroup">
              {tasks.map(task => (
                <button 
                  key={task} 
                  role="radio"
                  aria-checked={activeTask === task}
            ...
            <div className="mb-2 flex items-center justify-between px-2">
              <label htmlFor="custom-prompt" className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Analysis Prompt</label>
              <button className="text-[9px] text-indigo-400 font-bold cursor-pointer hover:text-indigo-600" onClick={() => setCustomPrompt(taskPrompts[activeTask])}>Reset Default</button>
            </div>
            <textarea 
              id="custom-prompt"
              value={customPrompt} 
              onChange={(e) => setCustomPrompt(e.target.value)} 
              className="w-full p-6 bg-slate-50 border border-slate-200 rounded-3xl text-xs font-bold h-32 resize-none mb-8 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all text-slate-600 leading-relaxed" 
            />
            ...
            {result && (
              <article className="mt-10 p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 animate-in slide-in-from-bottom-6 duration-700">
                <header className="flex items-center space-x-2 mb-6 opacity-40">
                  <div className="w-1 h-3 bg-indigo-600 rounded-full"></div>
                  <h3 className="text-[10px] font-black uppercase tracking-widest">Engineering Report</h3>
                </header>
                <div className="prose prose-sm prose-slate max-w-none font-medium leading-relaxed">
      ...
      {mode === 'HISTORY' && (
        <section className="space-y-4 pb-20">
           <h2 className="sr-only">Activity History</h2>
           {history.map(item => (
